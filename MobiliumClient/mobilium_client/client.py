...
class MobiliumClientNamespace(AsyncClientNamespace):
    ...
    async def on_message(self, data):
        if MessageDeserializer.start_driver_response(data):
            message = MessageDataFactory.install_app_request(self.device_udid, './app.ipa')
            await self.send(message)
        elif MessageDeserializer.install_app_response(data):
            message = MessageDataFactory.execute_test_request('com.silvair.commissioning.test.dev')
            await self.send(message)
        elif MessageDeserializer.execute_test_response(data):
            message = MessageDataFactory.uninstall_app_request(self.device_udid, 'com.silvair.commissioning.test.dev')
            await self.send(message)
